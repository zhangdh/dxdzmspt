...
import com.coffice.util.excel.ExcelUpload.ExcelConf;

import com.oreilly.servlet.MultipartRequest;

import edu.emory.mathcs.backport.java.util.LinkedList;

public class ExcelUpload extends BaseUtil {
		...
		try {
			...
			if (rows > 0 && cols > 0) {
				for (int i = 0; i < cols; i++) {
					...
					colTitleNameMap.put("colName_" + i, cellString);
			...
			int colNameSqlNum = 0;
			// add by hanxc at 20101216循环URL合并，是URL传递的参数 开始
			for (String keyString : urlPara.keySet()) {//URL传递参数
				urlColName = urlColName + keyString + ",";
				urlColValue = urlColValue + ":url" + colNameSqlNum + ",";
				map.put("url" + colNameSqlNum, urlPara.get(keyString));
				colNameSqlNum++;
			}
			...
			//add by hanxc at 20101216循环列，和URL合并，是excel第几列和URL传递的参数 结束
		
			
			colNameSqlNum = 0;
			//add by hanxc at 20101216循环读取excle数据 开始 循环列 在循环行
			...
			for (int j = 0; j < cols; j++) { // 循环列
				...
				//String excelColName = (String) colNameList.get(colNameSqlNum);
				String excelColName = (String) colTitleNameMap.get("colName_"+colNameSqlNum);//update yan_jwei 2010-10-18用conNameSqlNum获取出现数组越界，改从map获取
				if(excelColName==null||excelColName.trim().equals(""))continue;//如果列名为空就不再加入到数据库中
				
				String ywClassPath = excelConf.getffyzType(excelColName);
				if (ywClassPath.equals("")) { //update by yanjiawei 2010-9-16 增加处理读取excel能够添加数据库多列的能力
					String midcolname=(String) colList.get(colNameSqlNum);
					if(midcolname!=null&&!midcolname.trim().equals("")){//add by yan_jwei 2010-10-18该列不存在就不导入数据库
					String[] midcolnamearr=midcolname.split(",");
					colName = colName + colList.get(colNameSqlNum) + ",";
					if(midcolnamearr.length>1){
						for(int w=0;w<midcolnamearr.length;w++){
							colValue = colValue + ":col" + colNameSqlNum+"["+w + "],";
						}
					}else{
					colValue = colValue + ":col" + colNameSqlNum + ",";	
					}
					}
				}
				colNameSqlNum++;
			...
			//拼装查询sql，查询的字段需保存到数据库中，和数据库字段对应 开始
			*//**
			List queryList = new ArrayList();
			String querySql = excelConf.getzddy("query").toLowerCase();
			
			if(!"".equals(querySql)&&querySql!=null){
				String queryZd = querySql.substring(querySql.indexOf("select"),querySql.indexOf("from"));//select sql 查询字段，保存到数据库中
				String[] qZd = queryZd.split(",");
				for(int i=0;i<qZd.length;i++){
					//colName = colName+qZd[i]);
					colValue = colValue+":query"+i;
				}
				String queryTj= querySql.substring(querySql.indexOf("where"),querySql.length());//select where 条件，替换成传递的参数
				String[] tj = queryTj.split("and");//zt_dm={#} and a = 1
				for(int i=0;i<tj.length;i++){
 				...
 				for (String keyString : queryPara.keySet()) {//URL传递参数
					String key = keyString;
					String value = queryPara.get(key);
					
				}
			}
			**//*
			//拼装查询sql，查询的字段需保存到数据库中，和数据库字段对应 结束
			...
			endTime = System.currentTimeMillis();
			System.out.println("从数据库取值程序运行时间： " + (endTime - startTime)	+ "ms");
			...
			for (int i = 0; i < rows; i++) { // 循环行
				...
				for (int j = 0; j < cols; j++) { // 循环列
					...
					callArgMap.put(excelColName, cellString);
					if (!sqlString.equals("")) { 
						Object object = null;
						Map tempqzMap =null;
						try {
							//add by yan_jwei 原来由读取数据库改为从从内存map获取
							tempqzMap=(Map)midvalueMap.get(cellString);//字段取值后的数据，根据excel文件匹配数据，放到tem中pqzMap
							object = null;
							if(tempqzMap!=null){
								if(excelCol.indexOf(",")==-1){
									object = tempqzMap.get((String) excelCol);
								}
							}
						} catch (Exception e) {
							object = new String("");
						...
						String[] midcolnamearr=excelCol.split(",");
						if(midcolnamearr.length>1&&tempqzMap!=null){
							for(int w=0;w<midcolnamearr.length;w++){
								map.put("col" + j+"["+w+"]", tempqzMap.get(midcolnamearr[w]));
							}
						}else{
							map.put("col" + j, object);
						}
						colNum++;
				...
				if (!rowVildatePass) { // 本行验证没通过
					...
					try {
						...
						tgList.add(((HashMap)map).clone());//add by yan_jwei 把通过验证的数据存放到list，验证完后批量插入数据库
			...
			endTime = System.currentTimeMillis();
			System.out.println("结束******程序运行时间： " + (endTime - startTime)
					+ "ms");
			log.info("批量导入数据成功");
			if(msg.equals("")){
				  msg = "保存成功";
	...
	*//**
	 ...
	 *//*
	public String batchupdate(final String sql,final String cansql,final List list,final NamedParameterJdbcTemplate namedParameterJdbcTemplate,final SqlRowSetMetaData sqlRowSetMetaData){
	...
	*//**
	...
	public Map reSubExcelData(HttpServletRequest request) {
		...
		try {
			...
			String urlColName = "", urlColValue = "";
			int colNameSqlNum = 0;
			for (String keyString : urlParaMap.keySet()) {
				urlColName = urlColName + keyString + ",";
				urlColValue = urlColValue + ":url" + colNameSqlNum + ",";
				map.put("url" + colNameSqlNum, urlParaMap.get(keyString));
				colNameSqlNum++;
			}
			String colName = zhujian + ",", colValue = ":" + zhujian + ",";
			if (zhujian == null || zhujian.equals("")) {
			...
			for (int i = 0; i < rows; i++) { // 循环行
				...
				for (int j = 0; j < cols; j++) { // 循环列
					...
					callArgMap.put(excelColName, cellString);
					if (!sqlString.equals("")) { 
						sqlString = sqlString.replace("{@}", cellString);
						Object object = null;
						Map tempqzMap =null;
						try {
							//add by yan_jwei 原来由读取数据库改为从从内存map获取
							tempqzMap=(Map)midvalueMap.get(cellString);
							object = null;
							if(tempqzMap!=null){
								if(excelCol.indexOf(",")==-1){
									object = tempqzMap.get((String) excelCol);
						...
						if(midcolnamearr.length>1&&tempqzMap!=null){
							for(int w=0;w<midcolnamearr.length;w++){
								map.put("col" + colNum+"["+w+"]", tempqzMap.get(midcolnamearr[w]));
							}
						}else{
...
public Map queryZdqzSql(String sql){
	...
	public Map subAgoData(HttpServletRequest request) {
		...
		try {
			...
			sqlStr.append("insert into " + excelConf.bmc + " select * from "
					+ excelConf.bmcTemp);
	...
	public Map cancelData(HttpServletRequest request) {
		...
		try {
			...
			map.put("cellnames", "");
			oplog.info("批量导入取消已经导入的数据成功");
			msg = "保存成功";
		} catch (Exception e) {
			e.printStackTrace();
			String message=new StringBuffer().append("批量导入取消已经导入数据时出现异常:").append(e.toString()).toString();
	        log.error(message);
			msg = "保存失败";
		}
		return map;
	...
	*//**
	 ...
	 *//*
	void clearData(String uploadLishiGuid, int hisState, ExcelConf excelConf) {
		try {
