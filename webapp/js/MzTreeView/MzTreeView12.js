...
function MzTreeView(Tname)
{
  ...
  this.selectedIds='';
  this.selectedNames='';
	//0-选择人员 1-选择部门 2-选择部门+岗位
  this.oType='';
  this.parentIsChecked="0";//选择子节点的时候是否选择父节点 yanggc
  this.node["0"] =
  {
    "id": "0",
    "path": "0",
...
MzTreeView.prototype.nodeInit = function(sourceIndex, parentId)
{
  ...
  //xul(d:部门、j:岗位、p:人员、1-系统菜单 2-功能按钮)
  var flag=	this.getAttribute(source, "flag");
  var yhid=	this.getAttribute(source, "yhid"); //对应用户树的lxid   yanggc
  var xh = this.getAttribute(source, "xh"); //排序序号   yanggc 2010-3-9
...
MzTreeView.prototype.openAll = function()
{
...
MzTreeView.prototype.closeAll = function()
{
  ...
  while(e.id != "0")
  {
    ...
    else
    {
      if(e.hasChild && !isdo)
      {
        if(e.id!=1)//根节点不再折叠 --yanggc modify on 2010-08-21
        this.expand(e.id);
        t = false;
        for(var i=0; i<e.childNodes.length; i++)
        {
...
{  
    ...
    if(pckbox.checked){
     this.expandAllSon(id);//展开所有子节点
    }

	if(tcn.flag=="j"){//岗位选择 不考虑父子连带关系
    if (pckbox.checked){
       if(this.selectedIds.indexOf(tcn.sid)<0){
      this.createNodesNid(this.selectedIds,tcn);
     }
   }else{
    this.selectedIds=this.selectedIds.replace(tcn.sid + ',', '');
  }
   this.returnSNodeSelectedNames(pckbox,id);
 }else{
      this.selChild(id);
      if(this.parentIsChecked=="1"&&tcn.parentId!="1"){//yanggc modify on 2009-6-6
      if(tcn.flag=="2"&&pckbox.checked||tcn.flag!="2"){
       this.selParent(id);
       }
      }
  }
...
MzTreeView.prototype.selChild = function(id)
{
  ...
  var pckboxID = this.name +"_checbox_"+ id;
  var pckbox  = this.getElementById(pckboxID);
  /*yanggc modify 2010-03-11
	if (pckbox.checked){
    if((tcn.flag!="p"&&this.selectedIds.indexOf(tcn.sid)<0)||(tcn.flag=="p"&&this.selectedIds.indexOf(tcn.yhid)<0)){
      this.createNodesNid(this.selectedIds,tcn);
    }
  }else{
  	if(tcn.flag=="p") //选择用户 yanggc
  	    this.selectedIds=this.selectedIds.replace(tcn.yhid + ',', '');
  	else   
       this.selectedIds=this.selectedIds.replace(tcn.sid + ',', '');
  }*/
	//处理子节点
	...
	//alert(this.selectedNames);
};


MzTreeView.prototype.createNodesNid = function(nIds,tcn) {
    var nId="";
    var sourceIndex = tcn.sourceIndex;
    if(sourceIndex.indexOf("0_")!=0){//不考虑根节点 yanggc modify on 2009-5-3
	 nId=tcn.sid;
	 
	if(tcn.flag=="p"){//选择人员 yanggc modify on 2009-2-12 
	 if(tcn.icon=="man"){
	  if(nIds==""){
		    this.selectedIds=tcn.yhid+",";
		  }else{
		    this.selectedIds=nIds+tcn.yhid+",";
		  }
	 }
	}
	/*if(this.oType=="0"){//人员
		if(tcn.flag=="p" || tcn.flag==''){
		  if(nIds==""){
		    this.selectedIds=nId+",";
		  }else{
		    this.selectedIds=nIds+nId+",";
		  }
		}
	}else if(this.oType=="1"){
		if(nIds==""){
	    this.selectedIds=nId.substring(3)+",";
	  }else{
	    this.selectedIds=nIds+nId.substring(3)+",";
	  }
	}*/else{
		  if(nIds==""){
		    this.selectedIds=nId+",";
...
MzTreeView.prototype.returnSNodeSelectedNames = function(o,nId) {
/*
  var parentId=this.node[nId].parentId;
  var pnode = this.node[nId].parentNode;
  var str=this.node[nId].T + '';
  ...
  for(var i=strarray.length-1;i>=0;i--){
    if (i>0)
      dept=dept + strarray[i] + '\\';
    else
      dept=dept + strarray[i];
  }
  dept=dept+",";
  if (o.checked){
    if(this.selectedNames.indexOf(dept.substring(dept.lastIndexOf("\\")+1,dept.length))<0){ 
      this.selectedNames=this.selectedNames+dept+'\r\n';
    }
  }else{
    this.selectedNames=this.selectedNames.replace(dept+'\r\n','');
  }*/
    var tcn = this.node[nId];
    var sourceIndex = tcn.sourceIndex;
    if(sourceIndex.indexOf("0_")!=0){//不考虑根节点 yanggc modify on 2009-5-3
    var str=this.node[nId].T+"," ;
      if (o.checked){
    if(this.selectedNames.indexOf(str)<0){ 
      this.selectedNames=this.selectedNames+str;
    }
  }else{
    this.selectedNames=this.selectedNames.replace(str,'');
  }
  }
};
//展开某个节点的所有子节点--yanggc
MzTreeView.prototype.expandAllSon = function(id){
   var tcn = this.node[id];
  if(!tcn.isExpand){
  this.expand(id);
  }
  if(tcn.hasChild)
	{
	  for(var i=0; i<tcn.childNodes.length; i++)
	  {
     this.expandAllSon(this.node[id].childNodes[i].id);
	  }
	}
};
//对Click函数的另一种处理：不执行节点的js方法   yanggc on 2009-12-09
MzTreeView.prototype.Click2 = function(sourceId, defer)
{
  try {

  if (!defer)
  {
    setTimeout(this.name +".Click2('"+ sourceId +"', true)", 100);
    return;
  }
  var path = this.getPath(sourceId);
  if(path[0]!="0")
    ...
    if(root.hasChild)
    {
      ...
      for (var k=0; k<root.childNodes.length; k++)
      {
        if (root.childNodes[k].sourceIndex == sourceIndex)
        {
          root = root.childNodes[k];
          if(i<len - 1) this.expand(root.id, true);
          else this.focusClickNode2(root.id);
          break;
        }
      }
    }
...
MzTreeView.prototype.focusClickNode2      = function(id)
{
 try {
...
MzTreeView.prototype.nodeClick2 = function(id)
{
  var source = this.N[this.node[id].sourceIndex];
 // eval(this.getAttribute(source, "C"));
  return !(!this.getAttribute(source, "url") && this.url=="#");
