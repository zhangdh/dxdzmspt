...
function openAddProcess() {
   window.showModalDialog(XiorkFlowWorkSpace.BASE_PATH+"oswf/graphicdesigner/add_process.jsp", processList, "dialogWidth:800px;dialogHeight:600px;center:yes;help:no;scroll:no;status:no;resizable:yes;");
    //window.showDialog("addprocess.html", processList, "dialogWidth:800px;dialogHeight:600px;center:yes;");
}

//浏览
function viewProcess(name) {
    window.showModalDialog(XiorkFlowWorkSpace.BASE_PATH+"/oswf/graphicdesigner/view_process.jsp?name=" + name, processList, "dialogWidth:800px;dialogHeight:600px;center:yes;help:no;scroll:no;status:no;resizable:yes;");
	//window.showDialog("viewprocess.html?name=" + name, processList, "dialogWidth:800px;dialogHeight:600px;center:yes;");
}

//编辑
function updateProcess(name,processWorkId) {
    window.showModalDialog(XiorkFlowWorkSpace.BASE_PATH+"/oswf/graphicdesigner/update_process.jsp?name=" + name+"&processWorkId="+processWorkId, processList, "dialogWidth:800px;dialogHeight:600px;center:yes;help:no;scroll:no;status:no;resizable:yes;");
}
...
ProcessList.prototype.insertProcessRow = function (processName,processWorkId) {
    if (!processName) {
        return;
    }
    ...
    ProcessList._insertProcessRow(processRow, processName, processName,processWorkId);
};
ProcessList._insertProcessRow = function (row, text, processName, processWorkId,viewable, delalbe, editable) {
    ...
    if (editable) {
        ...
        editA.href = "updateprocess.html?name=" + processName+"&processWorkId="+processWorkId;
        //编辑
        editA.innerText = "\u7f16\u8f91";
        ...
        editA.onclick = function () {
            updateProcess(processName,processWorkId);
            return false;
        };
    } else {
        editCell.innerText = " ";
...
ProcessListAjax.prototype.processXMLHttpRequest = function (httpRequest) {
    ...
    for (var i = 0; i < fileNodes.length; i++) {
        ...
        var processWorkId = fileNode.getAttribute("workId");
         //alert(processWorkId);
        this.processList.insertProcessRow(processName,processWorkId);
    }
};
