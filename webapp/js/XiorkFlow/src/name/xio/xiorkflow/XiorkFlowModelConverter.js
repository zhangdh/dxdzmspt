...
XiorkFlowModelConverter.convertModelToXML = function (model) {
    ...
    //权限
    var rolesNode = doc.createElement("roles");
    workflowProcessNode.appendChild(rolesNode);
    var size=model.selectedRoleModels.size();
    for (var i = 0; i < size; i++) {
  	 	 var role = XMLDocument.newDomcument();
    	 var roleNode = role.createElement("role");
    	 
    	 var role = model.selectedRoleModels[i];
         roleNode.setAttribute("roleId", role.getId());
         roleNode.setAttribute("roleName", role.getName());
...
//添加权限
XiorkFlowModelConverter.convertRoleModelToXML = function (roleModel, doc) {
    var roleNode = doc.createElement(XiorkFlowModelConverter.NODE_ROLE);

    //
    roleNode.setAttribute(XiorkFlowModelConverter.ATTR_ROLE_ID, roleModel.getID());
    roleNode.setAttribute(XiorkFlowModelConverter.ATTR_ROLE_NAME, roleModel.getText());
    return roleNode;
};
//
...
//role
XiorkFlowModelConverter.NODE_ROLE = "role";
XiorkFlowModelConverter.ATTR_ROLE_NAME = "roleId";
XiorkFlowModelConverter.ATTR_ROLE_ID = "roleName";