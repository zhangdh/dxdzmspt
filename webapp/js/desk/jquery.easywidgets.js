...
(function($){
  ...
  function PrepareSortableHeaders(sortableItems, settings){
	...
	function getCookie(acptname) {
	var cookies=document.cookie;
	var oneCookie=cookies.split(";");
	var name='';
	for(var i=0;i<oneCookie.length;i++){
		var fenge=oneCookie[i].split("=");
		if(fenge[0]==acptname){
			name=fenge[1];
			if(acptname==" ew-collapse"){
				name=unescape(name);
				if(typeof(fenge[1])=="undefined"){
					name="";
				}
			}
		}
  ...
  function PrepareSortablePlaces(sortableItems, settings){
    ...
    $(s.selectors.places).sortable({
      ...
      stop : function(e, ui){
        ...
        var layOut= getCookie(' ew-position');
        var delItem= getCookie(' ew-close');
        
        if(typeof(delItem)=="undefined" || delItem==""){
			delItem='无'; 
		}
		if(sys_yhid=="admin"){
			if (window.confirm("改变设置会影响到其它用户默认显示方式,确定修改吗?")==false) return false;
			setLayOut(layOut,delItem);
		}else{
			setLayOut(layOut,delItem);
		}
...
function setLayOut(layOut,delItem){
  	sys_ajaxPost("/desk/default.do?method=set","layOutVal="+unescape(layOut)+"&delItem="+unescape(delItem),function(msg){
	});
}
        ...
        if(contentVisible){
          ...
          if(canCollapse){
            ...
            if(s.behaviour.useCookies && widgetId){
              ...
              //收缩
              var shouSuo=getCookie(" ew-collapse");
          	  sys_ajaxGet("/desk/default.do?method=setShouSuoFu","shouSuo="+shouSuo);
            }
          ...
          if(!widget.hasClass(s.options.closeConfirm)
            || confirm(s.i18n.confirmMsg)){
              if(haveId && s.behaviour.useCookies){
        		...
        		var delItem= getCookie(' ew-close');
				if(layOut==""){
					sys_ajaxPost("/desk/default.do?method=get","",function(json){
					layOut=json.initData;
					setLayOut(layOut,delItem);
					});
				}
				else{
